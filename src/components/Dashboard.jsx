...
function StatCard({ label, value, sub, className, icon: Icon, trend }) {
  ...
  return (
    <div className={`stat-card ${className}`}>
      ...
      <div
        className={`stat-value count-anim ${className === "income" ? "green" : className === "expense" ? "red" : ""}`}
      >
        {formatted}
      </div>
  ...
  return (
    <div className="tooltip-custom" style={{ padding: "10px 14px" }}>
      ...
      {payload.map((p) => (
        <div
          key={p.name}
          style={{ fontSize: 12.5, color: p.color, marginBottom: 2 }}
        >
          {p.name}: {fmt(p.value)}
        </div>
      ))}
...
export default function Dashboard() {
  const { transactions, totalIncome, totalExpenses, balance, setActiveTab } =
    useApp();
          ...
          {recent.map((t, i) => (
            ...
            >
              <div
                ...
                style={{ background: categoryColors[t.category] + "22" }}
              >
              ...
              <div
                className={`recent-amount ${t.type === "income" ? "amount-cell positive" : "amount-cell negative"}`}
              >
                {t.type === "income" ? "+" : "−"}₹
